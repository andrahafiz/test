...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form class="theme-form" enctype="multipart/form-data" method="POST" action="{{ route('customer.demand.store') }}">
                    @csrf
                    <div class="mb-3 row">
                        <label class="col-sm-3 col-form-label" for="inp_requestgas">Periode</label>
                        <div class="col-sm-9">
                            <input class="form-control" type="text" readonly value="{{ now()->format('Y-m-d') }}">
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label class="col-sm-3 col-form-label" for="inp_requestgas">Jumlah Permintaan Gas</label>
                        <div class="col-sm-9">
                            <input class="form-control @error('inp_requestgas') is-invalid @enderror" name="inp_requestgas" id="inp_requestgas" type="text" placeholder="Contoh : 550,10" value="{{ old('inp_requestgas') }}" autocomplete="false" autofocus>
...
<script>
    $("input[name='inp_requestgas']").on("keyup", function() {
        $("input[name='inp_requestgas']").val(formatRupiah(this.value, ' MMSCFD'));
    });
    /* Fungsi */
